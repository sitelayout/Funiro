	...
	new Swiper('.main-slider__slider', {
	   ...
	   pagination: {
		...
		navigation: {
		    ...
		    prevEl: '.slider-main .arrow-control__prev',
		    nextEl: '.slider-main .arrow-control__next',
		},
		// паралакс слайдер
		parallax: true,
	});
...
new Swiper('.slider-rooms__slider', {
	   ...
	   pagination: {
		...
		navigation: {
		    ...
		    prevEl: '.slider-rooms__slider .arrow-control__prev',
		    nextEl: '.slider-rooms__slider .arrow-control__next',
		},
		// паралакс слайдер
		parallax: true,
	});
...
new Swiper('.slider-tips__slider', {
	   ...
	   pagination: {
		...
		navigation: {
		    ...
		    prevEl: '.slider-tips__slider .arrow-control__prev',
		    nextEl: '.slider-tips__slider .arrow-control__next',
		},
		// паралакс слайдер
		parallax: true,
...
window.onload = function () {
    ...
    if (!targetElement.closest('.menu__item') && document.querySelectorAll('.menu__item.hover').length > 0) {
       const hoverArray  = document.querySelectorAll('.menu__item.hover');
       for (let i = 0; i < hoverArray.length; i++) {
       	hoverArray[i].classList.remove('hover');
       }
    }
}
   ...
   if (targetElement.classList.contains('actions-product__button')) {
      e.preventDefault();
      const productItemId = targetElement.closest('.item-product').dataset.pid;
      addToCart(targetElement, productItemId);
   }

   if (targetElement.closest('.cart-header_icon')) {
     if (document.querySelector('.cart-list').children.length > 0) {
      document.querySelector('.cart-header').classList.toggle('active');
     }
     e.preventDefault();
   } else if (!targetElement.closest('.cart-header') && 
      document.querySelector('.cart-header').classList.contains('active') && 
...
// coplfnm yjdые блоки товаров в html
 function loadProducts(data) {
   data.products.forEach( function(item) {
      ...
      let template = "";
      let templateStart = `<article data-pid="${productId}" class="products__item item-product">`;
      let templateEnd = `</article>`;

      let templateLabels = "";
      let templateLabelsStart = `<div class="item-product__labels">`;
      let templateLabelsEnd = `</div>`;
      let templateLabelsContent = "";
      if (productLabels) {
         productLabels.forEach(function(label) {
            const labelType = label.type;
            const labelValue = label.value;
            templateLabelsContent += `<div class="item-product__label item-product__label_${labelType}">${labelValue}</div>`
         })
      }
      templateLabels += templateLabelsStart + templateLabelsContent + templateLabelsEnd;

      let templateImage = `<a href="" class="item-product__image">
                             <img src="../img/products/productImage" alt="">
                           </a>`;

      let templateBodyStart = `<div class="item-product__body">`;
      let templateBodyEnd = `</div>`;

      let templateContent = `<div class="item-product__content">
                                 <h4 class="item-product__title">${productTitle}</h4>
                                 <div class="item-product__text">${productText}</div>
                              </div>`;

      let templatePrices = "";
      let templatePricesStart = `<div class="item-product__prices">`;
      let templatePricesEnd = `</div>`;
      let templatePricesContent = `<div class="item-product__price">${productPrice}</div>`;
      if (productOldPrice) {
         templatePricesContent += `<div class="item-product__price item-product__price_old">${productOldPrice}</div>`;
      }

      templatePrices += templatePricesStart + templatePricesContent + templatePricesEnd;

      let templateActions = `<div class="item-product__actions actions-product">
                                 <div class="actions-product__body">
                                    <a href="" class="actions-product__button btn btn_white">В корзину</a>
                                    <a href="${productShareUrl}" class="actions-product__link icon-share">Поделиться</a>
                                    <a href="${productLikeUrl}" class="actions-product__link icon-favorite">Нравится</a>
                                 </div>
                              </div>`;
      ...
      template += templateBodyStart;
      template += templateContent;
      template += templatePrices;
      template += templateActions;
      template += templateBodyEnd;
      template += templateEnd;
      

      const parent = document.querySelector('.products__items');
 ...
 function addToCart(button, productId) {
   if (!button.classList.contains('hold')) {
     ...
     productImageFly.setAttribute('class', 'imageFly ibg');
    ...
    function updateCart(button, productId, productAdd = true) {
        ...
        const cartIcon = cart.querySelector('.icon-cart');
...
// добавление 
        ...
        if (parseInt(cartQuantity.innerHTML) > 1) {
    ...
    // анимационный скролл галереи при движении по ней мышью
    const gallery = document.querySelector('.furniture__body');
    if (gallery && !isMobile.any()) {
      const furnitureItems = gallery.querySelector('.furniture__items');
      const furnitureColumn = gallery.querySelectorAll('.furniture__column');
      ...
      gallery.addEventListener('mousemove', function(e) {
         // получаем ширину
          // узнаем ширину галереи
      
         let galleryItemsWidth = 0;
         furnitureColumn.forEach(function(column) {
            galleryItemsWidth += column.offsetWidth;
         });

         const different = (galleryItemsWidth - gallery.offsetWidth)/2;

         const windowWidth = document.documentElement.clientWidth;

         // ноль по середине
         
        const coordX = windowWidth/2;
        positionX = e.screenX;
        coordXprocent = (positionX-coordX)/coordX;
 

        if (coordXprocent<0) {
           const move = different*(-coordXprocent);
           gallery.style.cssText = `transform: translateX(${move}px)`;
